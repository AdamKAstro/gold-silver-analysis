...
const ALPHA_VANTAGE_KEY = 'BIV80TT696VJIUL2'; // Use your Alpha Vantage API key
const MAX_RETRIES = 3;
...
async function fetchOverviewData(ticker, url) {
    if (!url || !(await validateUrl(url))) {
        await db.run(
            ...
            [ticker, "overview", url || "none", "invalid"]
        );
        return null;
    }
    ...
    try {
        ...
        console.warn(`Failed to fetch overview data from ${url} for ${ticker}: ${err.message}`);
        await db.run(
            "INSERT INTO url_verification_log (company_ticker, url_type, attempted_url, status) VALUES (?, ?, ?, ?)",
            [ticker, "overview", url, "fetch failed"]
        );
    }
    return null;
}
...
async function fetchYahooFinanceStockPrice(ticker, url) {
    if (!url || !(await validateUrl(url))) {
        await db.run(
            "INSERT INTO url_verification_log (company_ticker, url_type, attempted_url, status) VALUES (?, ?, ?, ?)",
            [ticker, "yahoo_finance", url || "none", "invalid"]
        );
        return null;
    }
    ...
    try {
        ...
        console.warn(`Failed to fetch Yahoo Finance data from ${url} for ${ticker}: ${err.message}`);
        await db.run(
            "INSERT INTO url_verification_log (company_ticker, url_type, attempted_url, status) VALUES (?, ?, ?, ?)",
            [ticker, "yahoo_finance", url, "fetch failed"]
        );
        return null;
    }
}
...
async function crossVerifyData(ticker, overviewData, pdfData, yahooData, alphaData) {
    ...
    const verifiedData = {
        ...
        resources_gold_moz: overviewData?.resources_gold_moz_from_overview || pdfData?.resources_gold_moz || 0,
        resources_silver_moz_from_overview: overviewData?.resources_silver_moz_from_overview || 0,
    ...
    if (stockPrices.length === 0) {
        ...
        if (variance / avgPrice > STOCK_PRICE_VARIANCE_THRESHOLD) {
            ...
            verifiedData.stock_price = stockPrices.find(s => s.source === 'Yahoo Finance')?.value || values[0]; // Prefer Yahoo if variance is high
        } else {
    ...
    // Cross-verify resources if available from both overview and PDF
    if (overviewData?.resources_gold_moz_from_overview && pdfData?.resources_gold_moz) {
        const variance = Math.abs(overviewData.resources_gold_moz_from_overview - pdfData.resources_gold_moz);
        if (variance > 0.1) {
            console.warn(`Resource variance for ${ticker}: Overview=${overviewData.resources_gold_moz_from_overview}, PDF=${pdfData.resources_gold_moz}, Variance=${variance}`);
            await fs.appendFile(LOG_FILE, `[${new Date().toISOString()}] Resource variance for ${ticker}: Overview=${overviewData.resources_gold_moz_from_overview}, PDF=${pdfData.resources_gold_moz}, Variance=${variance}\n`);
        }
    }
...
async function updateDatabase(ticker, data) {
    const fields = [
        'reserves_gold_moz',
        'resources_gold_moz',
        'resources_gold_moz_from_overview',
        'resources_silver_moz_from_overview',
        'production_total_au_eq_koz',
        'aisc_last_year_value',
        'stock_price',
        'last_updated_mining',
    ...
    const values = [
        ...
        data.verifiedData.resources_gold_moz_from_overview || 0,
        data.verifiedData.resources_silver_moz_from_overview || 0,
        ...
        data.verifiedData.last_updated_overview || null,
...
async function main() {
    try {
        ...
        for (const { TICKER: ticker, NAME: name } of companies) {
            ...
            const overviewUrl = urls.overview;
            ...
            if (overviewData) {
                miningData.resources_gold_moz_from_overview = overviewData.resources_gold_moz_from_overview;
                miningData.resources_silver_moz_from_overview = overviewData.resources_silver_moz_from_overview;
